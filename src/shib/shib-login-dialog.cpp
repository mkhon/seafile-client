...
{
    printf("new cookie for %s: %s = %s\n", url.toString().toUtf8().data(), cookie.name().data(), cookie.value().data());
    QString name = cookie.name();
    // if (url == url_ && name == kSeahubShibCookieName) {
...
Account ShibLoginDialog::parseAccount(const QString& cookie_value)
{
    ...
    printf("email: %s, token %s\n", email.toUtf8().data(), token.toUtf8().data());
